...
istream & operator >> ( istream & input , Date & value )
{
	cin >> value.month >> value.day >> value.year ;
	return input;
}
...
Date Date::operator * ( const Date & value ) const
{
	return Date ( month * value.month , day * value.day , year * value.month );
}

Date Date::operator + ( const Date & value ) const
{
	return Date ( month + value.month  , day + value.day, year + value .year );
}
